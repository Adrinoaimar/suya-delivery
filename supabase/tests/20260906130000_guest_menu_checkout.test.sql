...
select has_function('public', 'create_menu_order', array['uuid','jsonb','text','text','text','uuid','text','double precision','double precision'], 'menu order mantiene API y acepta guest');
select has_function('public', 'create_menu_order_with_customer', array['uuid','jsonb','text','text','text','text','uuid','text','text','double precision','double precision'], 'menu order acepta nombre guest');
select has_function('public', 'create_table_cash_order', array['uuid','jsonb','text','text','text','uuid','uuid','uuid','text'], 'table QR mantiene API y acepta guest');
select has_function('public', 'create_table_cash_order_with_customer', array['uuid','jsonb','text','text','text','text','uuid','uuid','uuid','text','text'], 'table QR acepta nombre guest');
...
select ok(
  (select prosecdef and proconfig = array['search_path=""']::text[]
   from pg_proc where oid = 'public.create_menu_order(uuid,jsonb,text,text,text,uuid,text,double precision,double precision)'::regprocedure),
  'menu order es SECURITY DEFINER con search_path vacío'
);
select ok(
  (select prosecdef and proconfig = array['search_path=""']::text[]
   from pg_proc where oid = 'public.create_table_cash_order(uuid,jsonb,text,text,text,uuid,uuid,uuid,text)'::regprocedure),
  'table QR order es SECURITY DEFINER con search_path vacío'
);

select ok(
  has_function_privilege('anon', 'public.create_menu_order(uuid,jsonb,text,text,text,uuid,text,double precision,double precision)', 'execute')
  and has_function_privilege('authenticated', 'public.create_menu_order(uuid,jsonb,text,text,text,uuid,text,double precision,double precision)', 'execute'),
  'menu order disponible para anon y authenticated'
);
select ok(
  has_function_privilege('anon', 'public.create_table_cash_order(uuid,jsonb,text,text,text,uuid,uuid,uuid,text)', 'execute')
  and has_function_privilege('authenticated', 'public.create_table_cash_order(uuid,jsonb,text,text,text,uuid,uuid,uuid,text)', 'execute'),
