-- A terminal payment attempt must not poison the next retry through the
-- global idempotency constraint. The order lock still makes concurrent calls
-- return the same pending/authorized attempt instead of creating duplicates.
...
comment on function public.create_payment_intent(uuid, text, text) is
  'Crea o reutiliza el intento activo; tras un intento terminal genera una clave de reintento única bajo lock de pedido.';
