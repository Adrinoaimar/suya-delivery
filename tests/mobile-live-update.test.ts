...
describe('actualizaciones nativas por rol', () => {
  it('no permite que Rider o Back Office instalen el bundle de Suya Cliente', () => {
    ...
    expect(apkBuilder).toContain('`${target.artifactPrefix}-${versionName}-code${versionCode}-debug.apk`');
    expect(apkVerifier).toContain("const suffix = `-${versionName}-code${versionCode}-${releaseBuild ? 'release' : 'debug'}.apk`;");
    ...
    expect(androidBuild).toContain('versionName "1.10"');
