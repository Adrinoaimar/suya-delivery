...
| P-05 | En verificación | Parser TS/Android valida formato y tope común; web focal 31/31; Android tiene regresiones nuevas pero no se pudo ejecutar por falta de JDK en este entorno | Ejecutar Android test/build y matriz real por versión de billetera |
| P-06 | En verificación | Binding guardado, cola separa eventos por binding; conteo y `queueFull` usan la vinculación activa; eventos antiguos permanecen cifrados y no se reenvían al nuevo restaurante | Ejecutar Android test/build y prueba física de rotación/revocación |
| P-07 | En verificación | Lock de cola, capacidad de 500 pendientes por binding, reintentos y estado `queueFull`; selección pending-first conserva todos los pendientes y solo recorta historia sincronizada; regresión nativa nueva sin ejecutar por falta de JDK | Ejecutar Android test/build y probar offline/reinicio/concurrencia en dispositivo |
...
| A-09 | En verificación | RPC account-aware, revocación/reactivación, rotación, auditoría sin secreto, token identificable y límite por dispositivo; `queueFull` derivado de eventos cifrados y ligado al binding activo; guardia de receptor inactivo | SQL/RLS/concurrencia en DB local |
