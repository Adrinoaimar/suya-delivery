...
- La auditoría P-05 detectó parsing parcial de `S/ 30.5`/`S/ 1,2345` y falta de límite web alineado. Web y Android ahora validan formato, rechazan continuación malformada y comparten el tope SQL de 100,000,000 céntimos. TDD web focal **18/18**; adaptadores/parser **28/28**; global **72 archivos / 342 pruebas**; typecheck, lint y diff pasan.
- Android queda pendiente de ejecutar tras este cambio porque el entorno actual no tiene `java`/JDK. No se regeneran ni se presentan APK nuevas hasta pasar Android test/build. Se preservan los cambios ajenos en `src/lib/routePlanner.ts`, `tests/route-planner.test.ts` y `output/`.
