...
| A-06 | En verificación | `OrderListOptions` offset/limit con máximo 50, orden estable `created_at`+`id`, proyección de listado sin `gateway_qr_payload`, detalle separado y botón `Cargar más`; `supabase-order.test.ts` 10/10 y `orders.test.ts` 11/11 | Confirmar índice/plan, RLS y paginación contra DB oficial |
