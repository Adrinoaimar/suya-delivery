...
- P-09: `20260916110000_context_bound_payment_evidence_hmac.sql` añade HMAC v2 privado ligado a receptor y tipo de evidencia, con contexto compartido entre declaración/observación/corrección, columnas separadas para compatibilidad legacy y exclusión del nuevo HMAC en auditoría; su contrato declara 13 aserciones. La ejecución oficial sigue pendiente.
...
| P-05 | En verificación | Parser TS/Android valida formato y tope común; web focal 21/21; `YapeNotificationListenerServiceTest` Android 11/11 en debug/release; `:app:test` y APK debug compilan | Matriz real por versión de billetera y prueba física |
| P-06 | En verificación | Binding guardado, cola separa eventos por binding; conteo y `queueFull` usan la vinculación activa; eventos antiguos permanecen cifrados y no se reenvían al nuevo restaurante; Android 11/11 y `:app:test` pasan | Prueba física de rotación/revocación |
| P-07 | En verificación | Lock de cola, capacidad de 500 pendientes por binding, reintentos y estado `queueFull`; selección pending-first conserva todos los pendientes y solo recorta historia sincronizada; Android 11/11 y `:app:test` pasan | Probar offline/reinicio/concurrencia en dispositivo |
...
| A-01 | En verificación | Rutas wallet atómicas; helper v2 calcula huella con canal/método/oferta/mesa/datos y rechaza conflicto; cliente reutiliza request y token guest; retries terminales no colisionan con la clave global; lectura/renovación de receptor exige restaurante coincidente | Ejecutar SQL/pgTAP y E2E real tras pérdida de respuesta/concurrencia |
...
| A-03 | En verificación | Test y bundle customer real: token sintético de 64 caracteres se conserva en sesión, `location.hash` queda vacío después de cargar; `GuestOrderPage` no pinta navegación/caché antes de autorización server-side; focal guest 23/23 | E2E con recarga, enlace en otro contexto y pérdida de respuesta sin duplicar pedido |
...
| A-08 | En verificación | APKs debug Rider/Backoffice/Wallet Observer generadas desde bundles actuales con versionCode 5/versionName 1.4; ZIP íntegro, metadatos y firma v2 pasan; hashes registrados en §76/E.md | Capturas, instalación/actualización y firma release |
| A-09 | En verificación | RPC account-aware, revocación/reactivación, rotación, auditoría sin secreto, token identificable y límite por dispositivo; `queueFull` derivado de eventos cifrados y ligado al binding activo; Android 11/11 en debug/release | SQL/RLS/concurrencia en DB local |
...
## Corrección P-03/P-11/A-01 — receptor ligado al restaurante en todos los caminos — 2026-09-16

- La auditoría de las migraciones recientes detectó que `get_payment_intent`, la reutilización de `create_payment_intent` y la renovación de `refresh_payment_intent` comprobaban `receiver_account_id` y `active`, pero dos caminos no comprobaban también que la cuenta perteneciera al restaurante del pedido. Una fila inconsistente podía dejar un QR cruzado o renovar hacia un destino ajeno.
- La migración forward-only `20260916150000_receiver_restaurant_binding_read_guard.sql` reemplaza las tres funciones con el mismo contrato, mantiene la autorización guest/owner, `SECURITY DEFINER`, locks, idempotencia, receptor activo y grants; añade la condición explícita `restaurant_id` en cada lectura ligada por UUID. La renovación obtiene el restaurante desde `orders`, no lo infiere del UUID.
- `supabase/tests/20260916150000_receiver_restaurant_binding_read_guard.test.sql` declara **15 aserciones** sobre firmas, vínculo de restaurante, cuenta activa, seguridad y permisos. El runtime oficial pgTAP/Supabase sigue pendiente por `ECONNREFUSED 127.0.0.1:54322`; no se ejecutaron pagos reales.
- La revisión estática conserva typecheck, lint y diff limpios; no cambia APKs ni producción. A-01/P-03/P-11 permanecen en verificación hasta ejecutar la migración, RLS y el caso real de cuenta cruzada en DB autorizada.

## Corrección P-11/U-08 — checkout fail-closed sin QR válido — 2026-09-16

- `PaymentInstructions` ya no indica «Abre Yape» cuando falta `qrPayload`, aunque exista una etiqueta de receptor. Sin QR válido y verificable, la pantalla ordena no pagar y contactar al restaurante; se conserva la revisión manual y no se crea una ruta alternativa ambigua.
- `tests/payment-instructions.test.tsx` añade la regresión del caso etiqueta-sin-QR y pasa **18/18**; la suite global queda en **71 archivos / 334 pruebas**. Build customer actualizado contiene el mensaje fail-closed y no el copy inseguro; smoke web actual **12/12**, typecheck, lint y secretos (**936 archivos**) pasan.
- Las APK debug Rider/Backoffice/Wallet Observer fueron regeneradas y verificadas con ZIP íntegro, firma v2, un firmante, SDK 36 y versión `1.4/5`; sus hashes no cambian porque la corrección pertenece al bundle customer web. No hubo pagos reales ni producción.

## Corrección P-11/U-08 — ocultar cobro después de autorización — 2026-09-16

- La auditoría detectó que un intento `authorized` ya no mostraba las acciones manuales, pero todavía podía mostrar el QR del negocio; en Culqi quedaba visible el bloque de checkout con el botón deshabilitado. Se corrigió el render para que una autorización server-side elimine QR, checkout, destinatario y acciones de pago, dejando solo estado y referencia auditables.
...
## Flujo final para goal continuo y corrección P-05 — 2026-09-16

- `E.md` añade §68 como bloque copiable para un goal de ejecución continua: no abandonar por duración, dificultad, errores normales, herramientas ausentes o primer enfoque fallido; registrar evidencia, cambiar hipótesis/herramienta, avanzar tareas independientes y detenerse únicamente ante orden expresa o autoridad/seguridad externa imprescindible. Incluye búsqueda por repositorios oficiales, revisión de licencia/mantenimiento/compatibilidad, ciclo TDD y gates de UI, arquitectura, RLS, pagos, Android y release.
- La solución de pagos permanece directa al restaurante, sin cookies propias ni pasarela obligatoria: ledger/intento server-side, QR oficial, declaración, observación no confiable y conciliación asistida. Una futura integración bancaria solo puede entrar como adaptador aislado con contrato, API, autenticidad, pruebas, cumplimiento y autorización; no se usa scraping ni se inventa una pasarela.
- La auditoría P-05 detectó parsing parcial de `S/ 30.5`/`S/ 1,2345` y falta de límite web alineado. Web y Android ahora validan formato, rechazan continuación malformada y comparten el tope SQL de 100,000,000 céntimos. TDD web focal **20/20**; adaptadores/parser **30/30**; global **72 archivos / 344 pruebas**; typecheck, lint y diff pasan.
- Android queda pendiente de ejecutar tras este cambio porque el entorno actual no tiene `java`/JDK. El build web con configuración pública sintética y `npm run verify:apps` sí pasan para customer, Rider y Backoffice; no se regeneran ni se presentan APK nuevas hasta pasar Android test/build. Se preservan los cambios ajenos en `src/lib/routePlanner.ts`, `tests/route-planner.test.ts` y `output/`.
- Bloqueos sin cambio: Supabase/Postgres oficial (`127.0.0.1:54322`), dispositivo/emulador, E2E financiero, accesibilidad/offline físicos, OTA privado, firma release y pagos reales. No se ejecutaron pagos, despliegues ni publicación.

## Corrección P-05 — paridad del prefijo corto de soles — 2026-09-16

- La auditoría comparativa TS/Android detectó que el parser web aceptaba `S 30.00`, pero lo clasificaba como USD porque solo reconocía `S/`, `S.` y `PEN`. Android ya lo interpretaba como PEN; la divergencia podía descartar o presentar mal una notificación válida de Yape.
- `currencyForPrefix()` ahora trata `S`, `S/`, `S.` y `PEN` como PEN. No añade dependencia, no cambia autoridad bancaria ni convierte una notificación en confirmación de pago.
- TDD: la prueba nueva falló antes y pasó después; `tests/yapeNotificationParser.test.ts` pasa **21/21**. Suite global posterior: **72 archivos / 347 pruebas**; typecheck, lint, secretos (**937 archivos**), `git diff --check` y `npm run build:apps` con configuración pública sintética pasan. Bundles aislados: customer, rider y backoffice.
- Commit fuente: `9e2ff1a` (`fix(payments): align short sol prefix parsing`). Se preservan sin incluir los cambios ajenos en `src/lib/routePlanner.ts`, `tests/route-planner.test.ts` y `output/`.
- No se genera APK nueva: el entorno sigue sin `java`/JDK. Android test/build, DB/RLS Supabase oficial, dispositivo, E2E financiero, offline/TalkBack, OTA privado, firma release y pagos reales permanecen pendientes.

## Corrección P-06/P-07/A-09 — cola aislada por vinculación — 2026-09-16

- La revisión detectó que `pendingEventCount()`, `queueFull` y el límite de captura contaban todos los eventos no sincronizados del archivo cifrado. Después de revincular el teléfono, 500 eventos del restaurante anterior podían bloquear la nueva cuenta, aunque el sincronizador ya evitaba enviárselos.
- El servicio ahora calcula capacidad y salud usando el `bindingId` activo. Los eventos antiguos se conservan cifrados, quedan ligados a su binding original y nunca se envían al token nuevo. La cola mantiene todos los pendientes y solo recorta historia ya sincronizada cuando necesita espacio.
- TDD: la nueva regresión de contrato pasa **7/7**; la suite global posterior pasa **72 archivos / 348 pruebas**. También pasan typecheck, lint, secretos (**937 archivos**), `git diff --check` y `npm run build:apps` con configuración pública sintética. Se añadieron regresiones Android para capacidad por binding y retención de pendientes.
- Commit fuente: `e6baf60` (`fix(wallet): scope queue health by binding`). El test/build Android no pudo ejecutarse por falta de `java`/JDK; no se genera APK nueva. DB/RLS, dispositivo, offline, concurrencia y rotación física permanecen pendientes.

## Verificación Android y APKs debug del goal — 2026-09-16

- El primer test nativo reprodujo dos defectos: la guardia de continuación monetaria no detectaba `S/ 1,2345` y las pruebas locales construían `org.json` con stubs Android. Se corrigió la guardia y se aisló el contrato puro de capacidad; el camino JSON de producción permanece cubierto por el wrapper. Commit `04e7e8c`.
- `bash android/gradlew -p android --offline test --no-daemon` pasa: observer **11/11** en debug y release, más ejemplo **1/1** por variante. Builds web aislados de Rider, Backoffice y Wallet Observer, Capacitor sync y `assembleDebug` pasan con configuración pública sintética.
- APK debug `1.4/5`, SDK 36, ZIP íntegro, firma v2 y un firmante: Rider `/tmp/Suya-Rider-debug-goal-20260916.apk` (**25,372,765 bytes**, `31afa370b9cc6f960a1e63127504903eed2dd787d3d0a719b365fb1f464e4a87`); Backoffice `/tmp/Suya-Backoffice-debug-goal-20260916.apk` (**25,240,784 bytes**, `7e6cf3d9afa3580b458c8477b0a0221bbf5424b87f55063da4a9a862c6f1948b`); Wallet Observer `/tmp/Suya-Wallet-Observer-debug-goal-20260916.apk` (**25,192,444 bytes**, `350bd9c86bd42272d6460a7f8e33a9dfea0b2c3e4b43074b3ca157c3d55d9998`).
- Son debug, no release. No secretos reales, pagos, pasarela, cookies propias, despliegue ni publicación. Sin `adb`/dispositivo quedan pendientes instalación/actualización, permisos, insets, TalkBack, fuente ampliada, offline, rotación física, DB/RLS oficial, E2E financiero, OTA privado y firma release.

## Corrección UI/privacidad — menú estático sin cookies ni servicios externos — 2026-09-16

- La reproducción del menú legado confirmó que `app.js` usa imports ESM, pero `index.html` lo cargaba como script clásico; eso detenía toda la interfaz antes de pintar. Ahora el entrypoint se carga como módulo y el QR se monta en un contenedor accesible.
- Se eliminó la dependencia de QR Server, Google Fonts y Unsplash. El QR se genera localmente con `kazuhikoarase/qrcode-generator`, vendorizado desde su repositorio oficial bajo licencia MIT y compatible con navegador estático; la prueba produce un SVG válido de 29 módulos. No se envía la URL del menú a terceros.
...
## Corrección P-03/P-11 — reutilización de intento ligada al restaurante — 2026-09-16

- La auditoría de las migraciones encadenadas encontró que `20260916130000_payment_intent_terminal_retry.sql` reemplazaba `create_payment_intent` y, en la rama de reutilización de un intento activo, comprobaba `active` pero no `restaurant_id = order.restaurant_id`. Un UUID de cuenta no debía bastar para exponer un QR de otro restaurante.
