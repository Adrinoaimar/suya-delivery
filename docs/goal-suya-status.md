...
- La primera revalidación posterior al reset devolvió 12 timeouts porque el preview se lanzó sin `--outDir` y respondió 404; se descartó como fallo del harness. Repetido con `npx vite preview --outDir dist/{customer,rider,backoffice}` y `--strictPort`, el smoke pasa **12/12** y los tres previews responden 200.
