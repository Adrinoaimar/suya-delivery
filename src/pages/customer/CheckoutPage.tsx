              ...
              {[
                {
                  value: 'cash' as const,
                  label: 'Efectivo',
                  description: isDeliveryOrder ? 'Al recibir' : 'En caja',
                  icon: Banknote,
                  disabled: false,
                },
                {
                ...
                return (
                  <button
                    ...
                    className={`rounded-btn border p-3 text-left transition ${selected ? 'border-suya-green bg-suya-lime-soft' : 'border-suya-border bg-white'} ${option.disabled ? 'cursor-not-allowed opacity-50' : 'hover:border-suya-green/60'}`}
                  >
                    <Icon
                      aria-hidden="true"
                      className={`h-5 w-5 ${selected ? 'text-suya-green' : 'text-suya-muted'}`}
                    />
                    <span className="mt-2 block text-sm font-semibold">{option.label}</span>
                    <span className="mt-0.5 block text-xs text-[#6B7076]">
                      {option.description}
                    </span>
                    {option.disabled && (
        ...
        <div className="space-y-3 lg:sticky lg:top-24">
          <Card>
            ...
            <Button type="submit" fullWidth size="lg" className="mt-4" disabled={submitting}>
              {submitting
                ? 'Procesando…'
                : `${method === 'cash' ? 'Confirmar pedido' : 'Continuar con pago'} · ${formatPrice(total)}`}
            </Button>
