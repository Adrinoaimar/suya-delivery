...
export default function MenuPage() {
  ...
  const unavailableSeo = (
    <SeoHead
      title="Menú no encontrado | Suya Delivery"
      description="Este menú no está disponible en Suya Delivery."
      path={`/menu/${slug}`}
      noIndex
    />
  );
  ...
  const cartOrigin = useCartStore((state) => state.origin);
  ...
  const sections = useMemo(
    () => ['Todos', ...new Set(products.map((product) => product.section))],
    [products],
  );
      ...
      <>
        ...
        <div className="shell py-10">
          <EmptyState
            ...
            description="El enlace no existe o el restaurante todavía no publicó su carta."
          />
        </div>
      </>
    );
  ...
  const menuFont =
    brand.fontFamily.trim().toLowerCase() === 'montserrat'
      ? 'Montserrat, system-ui, sans-serif'
      : 'Inter, system-ui, sans-serif';
  const logoImage = assetUrl(brand.logoUrl ?? store.logo);
  const heroImage = assetUrl(
    brand.heroImageUrl ||
      store.image ||
      (store.name.toLocaleLowerCase('es') === 'andá paya'
        ? '/images/stores/anda-paya/arroz-mariscos.webp'
        : null),
  );
  const theme = {
      ...
      >
        <header className="sticky top-0 z-20 border-b border-suya-carbon/5 bg-white/95 backdrop-blur">
          <div className="mx-auto flex h-[68px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            <Link
              to="/"
              aria-label="Volver a Suya Delivery"
            ...
            <div className="flex items-center gap-3">
              <span className="hidden text-[11px] font-semibold uppercase tracking-[0.18em] text-[#7D847E] sm:inline">
                Carta digital
              </span>
              <span
                role="status"
                className={`inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-bold ${open ? 'bg-[#0E6B44]/10 text-[#0E6B44]' : 'bg-suya-carbon/10 text-suya-carbon'}`}
              >
                <span
                  className={`h-2 w-2 rounded-full ${open ? 'bg-[#55A66C]' : 'bg-suya-carbon/40'}`}
                  aria-hidden="true"
                />
                {open
                  ? 'Recibiendo pedidos'
        ...
        <main
          id="contenido"
          className="mx-auto max-w-7xl lg:grid lg:grid-cols-[340px_minmax(0,1fr)] lg:gap-8 lg:px-8 lg:py-8"
        >
          <aside className="overflow-hidden border-b border-suya-carbon/5 bg-white lg:sticky lg:top-[92px] lg:self-start lg:rounded-3xl lg:border lg:shadow-card">
            <div className="relative h-48 bg-[var(--menu-primary)] sm:h-56 lg:h-60">
              {heroImage ? (
                ...
                <div className="flex h-full items-center justify-center bg-[var(--menu-primary)] px-6">
                  {logoImage ? (
                    <img
                      src={logoImage}
              ...
              <div
                ...
                aria-hidden="true"
              />
              <span className="absolute bottom-4 right-4 rounded-full bg-white/90 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-suya-carbon">
                Menú público
              </span>
            </div>
            <div className="relative px-4 pb-6 sm:px-6 lg:px-5">
              <div className="-mt-10 h-20 w-20 overflow-hidden rounded-2xl border-4 border-[#F8F5EE] bg-white shadow-card lg:border-white">
                {logoImage ? (
                  ...
                  <div className="grid h-full place-items-center text-2xl font-black text-[var(--menu-primary)]">
                    {store.name.slice(0, 1)}
                  </div>
                )}
              </div>
              <div className="mt-4 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--menu-primary)]">
                    {store.tags[0] ?? 'Restaurante'}
                  </p>
                  <h1 className="mt-1 font-display text-2xl font-bold leading-tight text-suya-carbon lg:text-[27px]">
                    {store.name}
                  </h1>
                </div>
                <span className="mt-1 shrink-0 rounded-full bg-suya-ivory px-2.5 py-1 text-[10px] font-semibold text-[#626963]">
              ...
              <div className="mt-4 flex items-center gap-2 border-t border-suya-carbon/5 pt-4 text-xs font-semibold text-[#626963]">
                <span
                  className="grid h-7 w-7 place-items-center rounded-full bg-suya-ivory text-[var(--menu-primary)]"
                  aria-hidden="true"
                >
                  ⌖
                </span>
          ...
          <section className="px-4 py-7 sm:px-6 lg:px-0 lg:py-0">
            <div className="mb-7 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-[var(--menu-primary)]">
                  Nuestra carta
                </p>
                <h2 className="mt-2 font-display text-3xl font-bold leading-tight tracking-[-0.03em] text-suya-carbon sm:text-4xl">
                  Elige algo rico
                </h2>
                <p className="mt-2 max-w-md text-sm leading-6 text-[#626963]">
                  Todo preparado al momento. Agrega tus favoritos y envía tu pedido en pocos pasos.
                </p>
              </div>
              <span className="text-xs font-semibold text-[#7D847E]">
            ...
            <div className="mb-5 flex items-center gap-3 rounded-2xl border border-suya-carbon/10 bg-white px-4 shadow-card transition-shadow focus-within:border-[var(--menu-primary)] focus-within:shadow-soft">
              ...
              <input
                ...
                className="h-14 min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-[#9AA09A]"
            ...
            <nav
              ...
              className="hide-scrollbar mb-8 flex gap-2 overflow-x-auto pb-1"
            >
              {sections.map((name) => (
                <button
                  key={name}
                  ...
                  className={`h-11 shrink-0 rounded-full border px-5 text-sm font-semibold transition-colors ${section === name ? 'border-[var(--menu-primary)] bg-[var(--menu-primary)] text-white shadow-sm' : 'border-suya-carbon/10 bg-white text-suya-carbon hover:border-[var(--menu-primary)]'}`}
                >
                  {name}
                </button>
              ))}
            ...
            <div className="mb-4 flex items-baseline justify-between gap-3">
              <h3 className="font-display text-xl font-bold text-suya-carbon">
                {section === 'Todos' ? 'Todos los platos' : section}
              </h3>
              <span className="text-xs font-medium text-[#7D847E]">
            ...
            {visible.length ? (
              <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
                {visible.map((product) => (
                  <ProductCard
                    ...
                    accentClassName="bg-[var(--menu-primary)] hover:brightness-90"
                    className="hover:border-[var(--menu-primary)]/30 border-suya-carbon/10 p-3.5 shadow-none transition-all hover:-translate-y-0.5 hover:shadow-soft"
                  />
                ))}
              </div>
            ) : (
              <EmptyState
                icon={<Utensils className="h-6 w-6" />}
                title={products.length ? 'No hay resultados' : 'Carta sin productos'}
                description={
                  products.length
                    ? 'Prueba otra búsqueda o categoría.'
                    : 'Este restaurante todavía no publicó platos disponibles.'
                }
              />
            )}
          </section>
        ...
        {ownCart && (
          <div className="fixed inset-x-0 bottom-[calc(1rem+env(safe-area-inset-bottom,0px))] z-30 px-4 lg:inset-x-auto lg:right-8 lg:w-[420px] lg:max-w-[calc(100vw-4rem)]">
            <Link
              to="/cart"
              className="mx-auto flex min-h-14 max-w-xl items-center justify-between rounded-2xl bg-suya-carbon px-5 py-4 font-semibold text-white shadow-soft transition-transform hover:-translate-y-0.5"
            >
              <span className="flex items-center gap-2">
                <ShoppingBag className="h-5 w-5" />
                Ver pedido ({totals.count})
              </span>
        ...
        <ProductSheet
          ...
          onAdded={() => {
            // Conserva carrito delivery del mismo restaurante. Cambia de canal solo
            // para carrito vacío o después de reemplazar otro restaurante.
            if (items.length === 0 || cartStoreId !== store.id || cartOrigin === 'suya_menu') {
