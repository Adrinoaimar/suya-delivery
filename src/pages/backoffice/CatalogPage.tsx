...
export default function CatalogPage() {
  ...
  const menuUrl = (slug: string) => `${window.location.origin}/menu/${slug}`;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [rows, menuRows] = await Promise.all([
        ...
        Promise.all(visible.map(async (store) => [store.id, (await storeService.getMenuSettings(store.id)) ?? defaults(store)] as const)),
      ]);
