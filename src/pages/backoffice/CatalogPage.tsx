...
export default function CatalogPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [rows, menuRows] = await Promise.all([
        ...
        Promise.all(
          scoped.map(async (store) => {
            const current = await storeService.getMenuSettings(store.id);
            return [
              store.id,
              current
                ? { ...current, fontFamily: normalizeFontFamily(current.fontFamily) }
                : defaults(store),
            ] as const;
          }),
        ),
  ...
  const save = async (storeId: string) => {
    ...
    if (!next || !/^[a-z0-9-]{3,80}$/.test(next.slug)) {
    ...
    try {
      ...
      notificationService.notify(
        saved.published
          ? 'Menú publicado y logo actualizado.'
          : 'Configuración guardada y logo sincronizado.',
        'success',
      );
    } catch (cause) {
      notificationService.notify(
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-2xl font-bold">Catálogo y publicación</h1>
          <p className="mt-1 text-sm text-[#68716C]">
            Personaliza tu marca, publica el menú y comparte su QR.
          </p>
        </div>
        <button
      ...
      {!loading && !error && owned.length === 0 && (
        <Card className="flex min-h-64 flex-col items-center justify-center text-center">
          <Utensils className="h-10 w-10 text-suya-muted" />
          <h2 className="mt-3 font-display text-lg font-bold">No hay restaurantes asignados</h2>
          <p className="mt-1 max-w-md text-sm text-[#68716C]">
            Un propietario o administrador debe vincular tu cuenta al restaurante.
          </p>
        </Card>
      )}
      <div className="space-y-5">
        {owned.map((store) => {
          const own = products.filter((product) => product.storeId === store.id);
          const menu = settings[store.id] ?? defaults(store);
          return (
            <Card key={store.id} className="overflow-hidden p-0">
              <div className="grid lg:grid-cols-[1fr_280px]">
                <div className="space-y-5 p-4 sm:p-6">
                  <div className="flex items-center gap-3">
                    {assetUrl(menu.logoUrl) ? (
                      <img
                      ...
                      <div className="grid h-14 w-14 place-items-center rounded-xl bg-suya-ivory">
                        <ImagePlus className="h-5 w-5 text-suya-muted" />
                      </div>
                    )}
                    <div>
                      <h2 className="font-display text-lg font-bold">{store.name}</h2>
                      <p className="text-xs text-[#68716C]">
                        {own.length} platos disponibles ·{' '}
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <label className="text-sm font-semibold">
                      ...
                      <input
                        ...
                        className="mt-1 w-full rounded-xl border border-suya-mist px-3 py-2 font-normal"
                      />
                    </label>
                    <label className="text-sm font-semibold">
                      Tipografía
                      <select
                        ...
                        onChange={(event) => update(store.id, { fontFamily: event.target.value })}
                        className="mt-1 w-full rounded-xl border border-suya-mist px-3 py-2 font-normal"
                      >
                    ...
                    <label className="text-sm font-semibold">
                      ...
                      <input
                        type="color"
                        value={menu.primaryColor}
                        onChange={(event) => update(store.id, { primaryColor: event.target.value })}
                        className="mt-1 h-11 w-full rounded-xl border border-suya-mist p-1"
                      />
                    </label>
                    <label className="text-sm font-semibold">
                      Color de acento
                      <input
                        type="color"
                        value={menu.accentColor}
                        onChange={(event) => update(store.id, { accentColor: event.target.value })}
                        className="mt-1 h-11 w-full rounded-xl border border-suya-mist p-1"
                      />
                    </label>
                    <label className="text-sm font-semibold">
                      URL del logo autorizado
                      <input
                        type="url"
                        inputMode="url"
                        value={menu.logoUrl ?? ''}
                        onChange={(event) => update(store.id, { logoUrl: event.target.value })}
                        placeholder="https://… o /brand/stores/logo.svg"
                        className="mt-1 w-full rounded-xl border border-suya-mist px-3 py-2 font-normal"
                      />
                    </label>
                    <label className="text-sm font-semibold">
                      ...
                      <input
                        type="url"
                        inputMode="url"
                        value={menu.heroImageUrl ?? ''}
                        onChange={(event) => update(store.id, { heroImageUrl: event.target.value })}
                        placeholder="https://… o /images/stores/portada.webp"
                        className="mt-1 w-full rounded-xl border border-suya-mist px-3 py-2 font-normal"
                      />
                    </label>
                    <label className="text-sm font-semibold">
                      ...
                      <input
                        ...
                        onChange={(event) => void upload(store.id, 'logo', event.target.files?.[0])}
                    ...
                    <label className="text-sm font-semibold">
                      ...
                      <input
                        type="file"
                        accept="image/jpeg,image/png,image/webp"
                        onChange={(event) => void upload(store.id, 'hero', event.target.files?.[0])}
                        className="mt-1 block w-full text-xs font-normal"
                      />
                    </label>
                  </div>
                  <div className="flex flex-wrap items-center gap-3">
                    <label className="inline-flex items-center gap-2 text-sm font-semibold">
                      <input
                        ...
                        onChange={(event) => update(store.id, { published: event.target.checked })}
                        className="h-4 w-4 accent-suya-green"
                      />
                      Menú público
                    </label>
                    <button
                      onClick={() => void save(store.id)}
                      disabled={saving === store.id}
                      className="press inline-flex items-center gap-2 rounded-xl bg-suya-green px-4 py-2 text-sm font-semibold text-white disabled:opacity-60"
                    >
                      <Save className="h-4 w-4" />
                      {saving === store.id ? 'Guardando…' : 'Guardar y actualizar QR'}
                    </button>
                  </div>
                  {own.length > 0 ? (
                    <ul className="grid gap-2 sm:grid-cols-2">
                      {own.slice(0, 6).map((product) => (
                        <li
                          key={product.id}
                          className="flex items-center gap-3 rounded-xl bg-suya-ivory p-3"
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-[#68716C]">S/ {product.price.toFixed(2)}</p>
                          </div>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <div className="rounded-xl border border-dashed border-suya-mist px-4 py-6 text-sm text-suya-muted">
                      Este restaurante todavía no tiene platos activos. Cuando el catálogo esté
                      cargado, aparecerá aquí y en la vista pública.
                    </div>
                  )}
                </div>
                <aside className="flex flex-col items-center justify-center gap-3 border-t border-suya-mist bg-[#F7FAF8] p-6 text-center lg:border-l lg:border-t-0">
                  <div className="rounded-2xl bg-white p-3 shadow-sm">
                    <QRCodeSVG value={menuUrl(menu.slug)} size={176} level="H" includeMargin />
                  </div>
                  <p className="text-xs font-semibold uppercase tracking-wider text-[#68716C]">
                    QR del menú
                  </p>
                  <p className="break-all text-xs text-[#68716C]">{menuUrl(menu.slug)}</p>
                  <a
                    href={menuUrl(menu.slug)}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 text-sm font-semibold text-suya-green"
                  >
      ...
      {loading && (
        <Card className="p-8 text-center text-sm text-[#68716C]">Cargando catálogo real…</Card>
      )}
    </div>
  );
}
