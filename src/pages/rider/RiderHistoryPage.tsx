        ...
        <ul className="space-y-2.5">
          {history.map((order) => (
            <li key={order.id} className="space-y-2.5">
              <div className="flex items-center justify-between gap-3 rounded-card bg-white p-4">
                ...
                <Button
                  ...
                  aria-controls={`rider-history-detail-${order.id}`}
                  onClick={() => setSelectedId(selectedId === order.id ? null : order.id)}
                >
                  {selectedId === order.id ? 'Ocultar' : 'Mostrar detalle'}
                </Button>
              </div>
              {selectedId === order.id && (
                <section
                  id={`rider-history-detail-${order.id}`}
                  className="rounded-card bg-white p-4"
                  aria-label={`Detalle ${order.code}`}
                >
                  <h2 className="flex items-center gap-2 font-display text-lg font-bold">
                    <Package className="h-5 w-5 text-suya-green" />
                    Detalle #{order.code}
                  </h2>
                  <p className="mt-1 text-sm text-[#6B7076]">
                    {order.storeName} · {formatDateTime(order.createdAt)}
                  </p>
                  <div className="mt-4 grid gap-4 sm:grid-cols-2">
                    <div>
                      <h3 className="font-display font-bold">Productos</h3>
                      ...
                      <p className="mt-2 font-bold">Total S/ {order.total.toFixed(2)}</p>
                    </div>
                    <div>
                      <h3 className="flex items-center gap-1 font-display font-bold">
                        <MapPin className="h-4 w-4 text-suya-green" />
                        Entrega
                      </h3>
                      <p className="mt-2 text-sm text-[#4A4F55]">{order.customer.name}</p>
                      <p className="text-sm text-[#6B7076]">{order.customer.address}</p>
                    </div>
                  </div>
                  {order.cancellationReason && (
                    <p className="mt-3 rounded-btn bg-suya-danger-soft p-3 text-sm text-suya-danger">
                      Motivo de cancelación: {order.cancellationReason}
                    </p>
                  )}
                  <div className="mt-4 border-t border-suya-mist pt-4">
                    <h3 className="mb-3 font-display font-bold">Seguimiento</h3>
